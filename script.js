...
function getFileIcon(filename) {
  if (filename.endsWith('.html') || filename.endsWith('.htm'))
    return '<span class="file-icon" style="color:#e34c26">&lt;&gt;</span>';
  if (filename.endsWith('.js') || filename.endsWith('.mjs') || filename.endsWith('.cjs'))
    return '<span class="file-icon" style="color:#f1e05a">JS</span>';
  if (filename.endsWith('.jsx'))
    return '<span class="file-icon" style="color:#61dafb">JS</span>';
  if (filename.endsWith('.ts'))
    return '<span class="file-icon" style="color:#3178c6">TS</span>';
  if (filename.endsWith('.tsx'))
    return '<span class="file-icon" style="color:#3178c6">TS</span>';
  if (filename.endsWith('.css'))
    return '<span class="file-icon" style="color:#563d7c">#</span>';
  if (filename.endsWith('.scss') || filename.endsWith('.less'))
    return '<span class="file-icon" style="color:#c76494">#</span>';
  if (filename.endsWith('.json'))
    return '<span class="file-icon" style="color:#cbcb41">{}</span>';
  if (filename.endsWith('.md') || filename.endsWith('.markdown'))
    return '<span class="file-icon" style="color:#519aba">MD</span>';
  if (filename.endsWith('.py'))
    return '<span class="file-icon" style="color:#3572A5">PY</span>';
  if (filename.endsWith('.svg'))
    return '<span class="file-icon" style="color:#ffb13b">SVG</span>';
  if (filename.endsWith('.yaml') || filename.endsWith('.yml'))
    return '<span class="file-icon" style="color:#cb171e">YML</span>';
  if (filename.endsWith('.sh') || filename.endsWith('.bash'))
    return '<span class="file-icon" style="color:#89d185">SH</span>';
...
term.write('\x1b[1;34mVSCode Online\x1b[0m v1.0.2\r\n');
  ...
  if (webcontainerInstance) {
    ...
    try {
      ...
      webcontainerInstance.on('server-ready', (port, url) => {
        printToTerminal(`> 🚀 Server ready at ${url} — loading preview...`, "#89d185");
        const previewPane = document.getElementById("preview-pane");
        const iframe = document.getElementById("live-iframe");
        if (previewPane && iframe) {
          ...
          iframe.src = url;
          // Vite may not have finished its first build when server-ready fires.
          // Reload once after 3 seconds to catch a 404 on first load.
          setTimeout(() => {
        ...
        for (const entry of entries) {
          ...
          try {
            ...
            if (subEntries.includes('package.json') && !subEntries.includes('node_modules')) {
              printToTerminal(`> Run: cd ${entry} && npm install`, "#cbcb41");
            }
          } catch { /* not a dir */ }
        }
        // Also check root level
        if (entries.includes('package.json') && !entries.includes('node_modules')) {
          printToTerminal(`> Run: npm install`, "#cbcb41");
        }
      } catch { /* ignore */ }
    }, 1000);
...
async function renderVirtualTree(path = '/', parentElement) {
  ...
  try {
    ...
    // WebContainer's dirent objects may expose type as a property rather than
    // a method in some versions — check both to be safe
    const isDir = (entry) => {
